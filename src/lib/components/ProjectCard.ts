...
export default function ProjectCard(DOM: HTMLElement, project: Project) {
    const ui = useTSElements(
        DOM,
        html`
        <div class="group relative overflow-hidden shadow-lg">
            <img src=${project.image} alt=${project.title}
                class="w-full h-auto transition-transform duration-500 group-hover:scale-110" />
        
            <!-- Overlay Content -->
            <div class="absolute inset-0 bg-black/70 transform translate-y-full opacity-0 
                                group-hover:translate-y-0 group-hover:opacity-100 
                                transition-all duration-500 ease-out flex flex-col justify-center items-center text-white">
